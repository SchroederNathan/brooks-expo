...
region 142–608.3pt, copy centred at 375.5pt against a true centre of 375.15pt.
Typing `ghost` still returns five joined hits with the summary row now directly
under the field.
...
subtracting one from the other is the app's job, and nothing in the API surface
suggests the question needs asking. The error is a silent 41–84pt offset that
...
this bug, and the symptom (slightly-too-high content) is exactly what a designer
...
- react-native-keyboard-controller could expose the keyboard's overlap with the
  *nearest view* rather than only the window-relative frame — e.g. a
  `useKeyboardOverlap(ref)` or a documented recipe for subtracting a tab bar or
  bottom inset. `KeyboardAvoidingView` solves the padding case; the "centre
  something in the remaining gap" case has no equivalent, and the naive version
  is wrong by exactly the tab bar's height.
